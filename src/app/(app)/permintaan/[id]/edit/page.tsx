...
import {
  ...
  Coffee, UtensilsCrossed, Sandwich, Wind, Droplets, MoreHorizontal, AlertCircle,
} from "lucide-react";
...
export default function EditPermintaanPage() {
    ...
    fetch(`/api/permintaan/${id}`)
      ...
      .then((data) => {
        if (data.status !== "PENDING") {
          toast.error("Permintaan yang sudah diproses tidak bisa diedit");
          router.replace(`/permintaan/${id}`);
          return;
        }
        ...
        setPemohonName(data.pemohon.nama);
        setPemohonQuery(data.pemohon.nama);
        setCatatan(data.catatan ?? "");

        const loaded = initItems();
        data.items.forEach((item: { jenis: string; qty: number; keterangan?: string }) => {
          loaded[item.jenis] = { checked: true, qty: String(item.qty), keterangan: item.keterangan ?? "" };
        });
        setItems(loaded);
      })
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href={`/permintaan/${id}`} className="btn-secondary p-2 rounded-xl">
          <ArrowLeft size={16} />
        </Link>
        <div>
          <h1 className="text-xl font-bold text-gray-900 tracking-tight">Edit Permintaan</h1>
          <p className="text-sm text-gray-400 mt-0.5">Perbarui detail permintaan konsumsi</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="time" value={jamMulai}
                onChange={(e) => { setJamMulai(e.target.value); clearError("jamMulai", "jamSelesai"); }}
            ...
            <div className="relative">
              ...
              {pemohonOpen && pemohonQuery.length > 0 && (
                <div className="absolute z-20 mt-1 w-full bg-white rounded-xl border border-gray-200 shadow-lg overflow-hidden">
                  {users
                    .filter((u) => u.nama.toLowerCase().includes(pemohonQuery.toLowerCase()))
                    .slice(0, 6)
                    .map((u) => (
                      <button
                        key={u.id}
        ...
        <div className="flex justify-end gap-3 pt-1 pb-6">
          <Link href={`/permintaan/${id}`} className="btn-secondary">Batal</Link>
